class Tile{
  ...
  int layer;
  int offsetX,offsetY;
  Tile(int r, int c, int size,double paddingRatio,color col,int imageIndex,boolean negative,int layer,int offsetX,int offsetY){
    this.r=r;
    this.c=c;
    this.size=size;
 ...
 ArrayList<Tile> split(){
     ArrayList<Tile> children=new ArrayList<Tile>();
      for(int rr=0;rr<2;rr++){
          for(int cc=0;cc<2;cc++){
              ...
              Tile child =new Tile(rr,cc,size/2,paddingRatio,col,childrenImageIndex,!negative,layer+1,childOffsetX,childOffsetY); 
              children.add(child);
          }
      }
      return children;
  ...
  Tile clone(){
  return new Tile(r,c,size,paddingRatio,col,imageIndex,negative,layer,offsetX,offsetY);
  }
}
